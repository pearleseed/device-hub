...
export const departmentsRoutes = {
  ...
  async getAll(): Promise<Response> {
    try {
      const departments = await db<
        Department & { user_count: number; equipment_count: number }
      >`
        SELECT d.*, 
  ...
  async delete(
    ...
    try {
      ...
      const counts = await db<{ user_count: number; equipment_count: number }>`
        SELECT 
      ...
      if (
        counts[0] &&
        (counts[0].user_count > 0 || counts[0].equipment_count > 0)
      ) {
        return jsonResponse(
          {
            success: false,
            error: "Cannot delete department with users or equipment",
          },
          400,
        );
      }
