...
import { ok, created, err, notFound, parseId, withAdmin } from "./_helpers";
...
export const devicesRoutes = {
  async getAll(request: Request): Promise<Response> {
    try {
      ...
      return err("Failed to get devices", 500);
    }
  },
  ...
  async getById(_: Request, params: Record<string, string>): Promise<Response> {
    try {
      ...
      return err("Failed to get device", 500);
    }
  },
  ...
  async getByCategory(_: Request, params: Record<string, string>): Promise<Response> {
    try {
      ...
      console.error("Get devices by category error:", e);
      return err("Failed to get devices", 500);
    }
  },

  async getByStatus(_: Request, params: Record<string, string>): Promise<Response> {
    try {
      ...
      console.error("Get devices by status error:", e);
      return err("Failed to get devices", 500);
    }
  },

  async getPendingDeviceIds(_: Request): Promise<Response> {
    try {
      // Get device IDs that have pending or approved borrow requests
      ...
      console.error("Get pending device IDs error:", e);
      return err("Failed to get pending device IDs", 500);
    }
  },
};
