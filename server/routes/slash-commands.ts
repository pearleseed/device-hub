...
export const slashCommandsRoutes = {
  ...
  async interactive(request: Request): Promise<Response> {
    try {
      const interactiveRequest: InteractiveMessageRequest = await request.json();

      console.log("[SlashCommands] Interactive request:", JSON.stringify(interactiveRequest, null, 2));
