...
export const renewalsRoutes = {
  ...
  async create(request: Request): Promise<Response> {
    try {
      ...
      if (borrowRequest.status !== "active") {
        return jsonResponse(
          { success: false, error: "Can only request renewal for active loans" },
          400,
        );
      }
      ...
      if (borrowRequest.user_id !== payload.userId) {
        return jsonResponse(
          { success: false, error: "You can only request renewal for your own loans" },
          403,
        );
      }
      ...
      if (newEndDate <= currentEndDate) {
        return jsonResponse(
          { success: false, error: "Requested end date must be after current end date" },
          400,
        );
      }
      ...
      if (existingRenewals[0].count > 0) {
        return jsonResponse(
          { success: false, error: "A pending renewal request already exists for this loan" },
          400,
        );
      }

      const reasonTrimmed = reason.trim();
      const currentEndDateStr = borrowRequest.end_date.toISOString().split("T")[0];
  ...
  async updateStatus(
    ...
    try {
      ...
      const validStatuses: RenewalStatus[] = ["pending", "approved", "rejected"];
      ...
      if (currentRenewal.status !== "pending") {
        return jsonResponse(
          { success: false, error: "Can only update status of pending renewal requests" },
          400,
        );
      }
      ...
      await db.begin(async (tx) => {
        ...
        if (status === "approved") {
          const requestedEndDate = currentRenewal.requested_end_date.toISOString().split("T")[0];
          await tx`
      ...
      // Trigger Mattermost notification for approved renewals
      if (status === "approved") {
      ...
      return jsonResponse({
        ...
        message: status === "approved" ? "Renewal approved" : "Renewal rejected",
      });
    } catch (error) {
  ...
  async getByBorrowRequest(
    ...
    try {
      ...
      const renewals = await db.unsafe<RenewalRequestWithDetails>(sql, sqlParams);
      return jsonResponse({ success: true, data: renewals });
    } catch (error) {
