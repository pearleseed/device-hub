      ...
      switch (action) {
        case NotificationAction.BORROW:
          if (!startDate || !endDate) {
            return jsonResponse(
              { success: false, error: "Start date and end date are required for BORROW" },
              400
            );
          }
        ...
        case NotificationAction.EXTEND:
          if (!previousEndDate || !newEndDate) {
            return jsonResponse(
              { success: false, error: "Previous and new end dates are required for EXTEND" },
              400
            );
          }
    ...
    try {
      ...
      if (success) {
        ...
        return jsonResponse(
          {
            success: false,
            error: "Failed to initialize notification service. Check configuration.",
          },
          500
        );
      }
