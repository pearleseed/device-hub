...
const CHANNEL_MESSAGES: Record<NotificationAction, string> = {
  [NotificationAction.BORROW]: "Your device borrowing request has been approved",
  [NotificationAction.RETURN]: "Your device return has been confirmed",
  [NotificationAction.RENEWAL]: "Your device borrowing renewal has been approved",
};
  ...
  switch (action) {
    ...
    case NotificationAction.RENEWAL:
      if (!params.previousEndDate || !params.newEndDate) {
        throw new Error("RENEWAL action requires previousEndDate and newEndDate");
      }
      return getRenewalDM({
        device,
...
export const ERROR_MESSAGES = {
  ...
  userNotFound: (username: string) =>
    `Mattermost user not found: @${username}`,
  ...
  wsConnectionError: (error: string) =>
