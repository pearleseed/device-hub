 ...
 */
function calculateRenewedDate(currentEndDate: string, presetValue: string): string {
 ...
 */
function parseDateRange(input: string): { startDate: string; endDate: string } | null {
...
async function handleBorrowConfirmation(
  ...
  try {
    ...
    return buildErrorMessage("Failed to create borrow request. Please try again.");
  }
}
...
async function handleRenewalDeviceSelection(
  ...
  const endDateStr = typeof borrowRequest.endDate === "string" 
  ...
  return buildRenewalDurationSelection(session.id, borrowRequest.deviceName, endDateStr);
}
...
async function handleRenewalConfirmation(
  ...
  try {
    ...
    return buildErrorMessage("Failed to create renewal request. Please try again.");
  }
}
