...
export async function handleSlashCommand(
  ...
  console.log(`[SlashCommands] Received command from @${user_name}: /device ${text}`);
  ...
  switch (parsed.action) {
    ...
    case CommandAction.RETURN:
      return handleReturnCommand(user_id, channel_id, appUserId, parsed.deviceId);

    case CommandAction.RENEWAL:
      return handleRenewalCommand(user_id, channel_id, appUserId, parsed.deviceId);
...
async function handleReturnCommand(
  ...
  const session = createReturnSession(mattermostUserId, channelId, borrowRequestId);
...
async function handleRenewalCommand(
  ...
  const session = createRenewalSession(mattermostUserId, channelId, borrowRequestId);
...
export async function handleInteractiveMessage(
  ...
  console.log(`[SlashCommands] Interactive action from user ${user_id}:`, context);
