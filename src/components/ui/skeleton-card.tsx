...
interface SkeletonCardProps { className?: string; }
...
export const SkeletonListItem: React.FC<SkeletonCardProps> = memo(({ className }) => (
  <div className={cn("flex items-center gap-4 p-4 rounded-lg border bg-card", className)}>
    <div className="h-16 w-16 rounded-lg bg-muted animate-shimmer shrink-0" />
...
export const SkeletonKPICard: React.FC<SkeletonCardProps> = memo(({ className }) => (
...
export const SkeletonTable: React.FC<{ rows?: number; className?: string }> = memo(({ rows = 5, className }) => (
  <div className={cn("rounded-lg border bg-card overflow-hidden", className)}>
...
export const SkeletonAvatar: React.FC<{ size?: "sm" | "md" | "lg"; className?: string }> = memo(({ size = "md", className }) => {
  const sizeClasses = { sm: "h-8 w-8", md: "h-10 w-10", lg: "h-16 w-16" };
  return <div className={cn("rounded-full bg-muted animate-shimmer", sizeClasses[size], className)} />;
});
