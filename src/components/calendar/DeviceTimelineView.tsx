...
import { getDevices, getUserById } from "@/lib/api";
...
export const DeviceTimelineView: React.FC<DeviceTimelineViewProps> = ({
  ...
  // Memoize filtered devices to prevent infinite loops
  ...
  // Memoize getDeviceBookings to prevent recalculation
  const getDeviceBookings = useCallback((deviceId: string) => {
  ...
  // Fetch users for visible bookings
  useEffect(() => {
    const fetchUsers = async () => {
      const uniqueUserIds = new Set<string>();
      filteredDevices.forEach((device) => {
        const deviceBookings = getDeviceBookings(String(device.id));
        deviceBookings.forEach((b) => uniqueUserIds.add(b.userId));
      });
  ...
  }, [filteredDevices, getDeviceBookings]);
                ...
                >
                  <div className="w-[140px] shrink-0 px-2 py-1.5 border-r border-border">
                    ...
                    <div className="text-[9px] text-muted-foreground leading-tight">
                      {device.assetTag || device.asset_tag}
                    </div>
                  </div>
