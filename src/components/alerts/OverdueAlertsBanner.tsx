...
import { useOverdueAlerts, OverdueItem } from '@/hooks/use-overdue-alerts';
...
export const OverdueAlertsBanner: React.FC<OverdueAlertsBannerProps> = ({ compact = false, className }) => {
  const { t } = useLanguage();
    ...
    return (
      <Alert className={cn("border-red-500 bg-red-500/10", className)}>
        ...
        <AlertDescription className="flex items-center justify-between">
          ...
          <Button variant="ghost" size="sm" asChild className="text-red-500 hover:text-red-600">
...
const OverdueCard: React.FC<{ item: OverdueItem }> = ({ item }) => {
  ...
  return (
    <div
      className={cn(
        "flex-shrink-0 w-64 p-4 rounded-lg border-2 transition-all hover:shadow-md",
        getSeverityColor(item.daysOverdue)
      )}
    >
      <div className="flex items-start justify-between mb-2">
        <span className="font-medium text-sm truncate flex-1">{item.device.name}</span>
        <Badge className={cn("ml-2 flex-shrink-0", getSeverityBadge(item.daysOverdue))}>
          {item.daysOverdue}d
        </Badge>
      </div>
